...
//! | `cache_stats`        | `CacheStatsRepr`     | 20 × i64 (4 × 5: metadata + statistics + column_index + offset_index caches) |
...
pub fn pack_cache_stats(mgr: &CustomCacheManager) -> CacheStatsRepr {
    let metadata_memory = mgr
        .get_memory_consumed_by_type(crate::cache::metadata_cache::CACHE_TYPE_METADATA)
        .unwrap_or(0) as i64;
    let statistics_memory = mgr
        .get_memory_consumed_by_type(crate::cache::metadata_cache::CACHE_TYPE_STATS)
        .unwrap_or(0) as i64;
...
mod tests {
    ...
    fn test_df_stats_buffer_too_small() {
        ...
        assert!(layout::BUFFER_BYTE_SIZE > 0) // 680 bytes (85 longs);
    }
