...
impl CacheAccessor<Path, CachedFileMetadataEntry> for MutexFileMetadataCache {
    ...
    fn put(&self, k: &Path, v: CachedFileMetadataEntry) -> Option<CachedFileMetadataEntry> {
        ...
        // (`datafusion-datasource-parquet/src/metadata.rs`). On wide schemas that
        // decoded index dominates the native heap and, since this is a shared LRU
        // keyed by path, also evicts the small footer-only entries the scan paths
        // depend on.
        //
        ...
        // query through the shared scoped cache
        // (`indexed_table::page_index_loader`), at query time only.
