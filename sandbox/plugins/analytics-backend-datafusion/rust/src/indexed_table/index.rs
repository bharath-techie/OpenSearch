...
pub trait RowGroupDocsCollector: Send + Sync + Debug {
    ...
    /// bitset. `Ok(None)` = unsupported by this backend (or the leaf has
    /// deletions) — the caller must fall back to
    /// [`collect_packed_u64_bitset`](Self::collect_packed_u64_bitset).
    ///
