...
struct IndexedStream {
    ...
    /// Full-schema indices requested by the query output, excluding columns read only for
    /// predicate evaluation. Per-RG delegation (Fix 9) adds the columns DataFusion still owns
    /// to this base immediately before creating the RG stream.
