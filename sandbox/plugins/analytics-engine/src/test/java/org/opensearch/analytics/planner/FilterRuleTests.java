...
public class FilterRuleTests extends BasePlannerRulesTests {
    ...
    public void testKeywordWithNormalizerEqualsIsLuceneOnly() {
        ...
        AnnotatedPredicate annotated = (AnnotatedPredicate) result.getCondition();
        assertEquals("Normalized keyword EQUALS is Lucene-only", List.of(MockLuceneBackend.NAME), annotated.getViableBackends());
        assertFalse(
        ...
        // (DelegationPossible) peer, so the combiner emits correctness delegation — never a
        // dual-viable performance leaf.
    ...
    public void testKeywordNoNormalizerEqualsStaysDualViable() {
        ...
        // (DelegationPossible) peer when narrowed onto the driving backend.
