...
public class SumValueAggregatorTests extends AbstractValueAggregatorTests {
    ...
    public void testMergeAggregatedValueAndSegmentValue_nullSegmentDocValue() {
        ...
        Long randomLong = randomLong();
        CompensatedSum initial = aggregator.getInitialAggregatedValue(new CompensatedSum(randomDouble1, 0));

        CompensatedSum resultWithNull = aggregator.mergeAggregatedValueAndSegmentValue(initial, null);
        assertEquals(randomDouble1, resultWithNull.value(), 0.0);

        CompensatedSum resultWithValue = aggregator.mergeAggregatedValueAndSegmentValue(initial, randomLong);
    ...
    public void testMergeAggregatedValueAndSegmentValue_nullInitialDocValue() {
        Long randomLong = randomLong();
        CompensatedSum initial = aggregator.getInitialAggregatedValue(null);
        CompensatedSum result = aggregator.mergeAggregatedValueAndSegmentValue(initial, randomLong);
    ...
    public void testMergeAggregatedValues() {
        ...
        CompensatedSum initial = aggregator.getInitialAggregatedValue(value1);
        CompensatedSum resultWithNull = aggregator.mergeAggregatedValues(null, initial);
        assertEquals(randomDouble1, resultWithNull.value(), 0.0);
