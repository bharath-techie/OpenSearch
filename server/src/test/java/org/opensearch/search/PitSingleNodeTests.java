...
public class PitSingleNodeTests extends OpenSearchSingleNodeTestCase {
    ...
    public void testCreatePIT() throws ExecutionException, InterruptedException {
        ...
        SearchResponse searchResponse = client().prepareSearch("index")
                .setSize(2).setPointInTime(new PointInTimeBuilder(pitResponse.pointInTimeId()).setKeepAlive(TimeValue.timeValueDays(1))).get();
    ...
    public void testMaxOpenPitContexts() throws Exception {
        ...
        SearchResponse pitResponse = null;
        for (int i = 0; i < SearchService.MAX_OPEN_PIT_CONTEXT.get(Settings.EMPTY); i++) {
            ActionFuture<SearchResponse> execute = client().execute(CreatePITAction.INSTANCE, request);
            pitResponse = execute.get();
            logger.info(SearchService.MAX_OPEN_PIT_CONTEXT.get(Settings.EMPTY) + " -- " + i);
        }
        ExecutionException ex = expectThrows(
                ExecutionException.class,
                () -> {
        ...
        assertTrue(ex.getMessage().contains("Trying to create too many Point In Time contexts. " +
                "Must be less than or equal to: [" + SearchService.MAX_OPEN_PIT_CONTEXT.get(Settings.EMPTY) + "]. " +
