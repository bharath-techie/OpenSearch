...
import org.opensearch.common.util.MovingAverageDouble;
...
//        );
    ...
    public void setWindowDuration(TimeValue windowDuration) {
        ...
        readLatencyObservations.set(new MovingAverageDouble(windowSize));
        writeLatencyObservations.set(new MovingAverageDouble(windowSize));
        queueSizeObservations.set(new MovingAverageDouble(windowSize));
    }

    private void recordUsage(IoUsageFetcher.DiskStats usage) {
        ioTimeObservations.get().record(usage.getIoTime()); // IO USE PERCENT
        readIopsObservations.get().record((long)usage.getReadOps()); //
        readKbObservations.get().record(usage.getReadThroughputInKB()); // TH
        ...
        queueSizeObservations.get().record(usage.getQueueSize());
    }
