...
public class StarTreeQuery extends Query implements Accountable {
    ...
    public StarTreeQuery(Map<String, List<Predicate<Long>>> compositePredicateMap,
        Map<String, List<String>> keywordList, Set<String> groupByColumns) {
    ...
    public Weight createWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost)
        ...
        return new ConstantScoreWeight(this, boost) {
            ...
            public Scorer scorer(LeafReaderContext context)
                ...
                for (String fieldInKeyList : keywordList.keySet()) {
                    SortedSetDocValues field = context.reader().getSortedSetDocValues(fieldInKeyList);
...
//                        Thread.currentThread().getName()));
                    ...
                    if (filter._result.maxMatchedDoc != -1) {
                        logger.info("Query ::: keywordOrdMap size : {}", 0);
                    } else {
            ...
            Map<String, List<Predicate<Long>>> getKeywordOrdMap(final LeafReaderContext context,
                final Map<String, List<String>> keywordList, final Map<String, SortedSetDocValues> keywordMap,
                final SortedSetDocValues field)
                throws IOException {
                ...
                for (Map.Entry<String, List<String>> keyword : keywordList.entrySet()) {
                    final List<Predicate<Long>> predicates =
                        keywordOrdMap.getOrDefault(keyword.getKey(), new ArrayList<>());
...
//                        logger.info("Query ::: ORD : {} , fieldOrd :{}" , ord, fieldOrd);
            ...
            Map<String, List<Long>> getKeywordOrdLongMap(final LeafReaderContext context,
                final Map<String, List<String>> keywordList, final Map<String, SortedSetDocValues> keywordMap,
                final SortedSetDocValues field)
                throws IOException {
                ...
                for (Map.Entry<String, List<String>> keyword : keywordList.entrySet()) {
                    ...
                    for (String val : keyword.getValue()) {
                        ...
                        //  logger.info("Query ::: ORD : {} , fieldOrd :{}" , ord, fieldOrd);
                        if (ord > 0) {
//                            System.out.println(String.format("Query ::: ORD : %d , fieldOrd : %d , IP : %s ,
//                            ContextSeg : %s, thread : %s " , ord, fieldOrd, val,
//                                info.info.name, Thread.currentThread().getName()));
//                            logger.info("Query ::: ORD : {} , fieldOrd :{} , IP : {} , ContextSeg : {}, thread " ,
//                            ord, fieldOrd, val,
//                                info.info.name, Thread.currentThread().getName());
                            predicates.add(ord);
                        }
